...
<script>
	function fn_search(currentPage,mem_id) {
		...
		frm.action = "${pageContext.request.contextPath}/mypage/resrveList?mem_id="+mem_id;//절대경로
	...
	function fn_delete(seqNo,memPoint,memId) {
		if(confirm("취소하시겠습니까?")){
			location.href = "${pageContext.request.contextPath}/mypage/resrveDelete?reservation_id=" + seqNo + "&mem_point=" + memPoint + "&mem_id=" + memId;
			...
			<form name="searchForm" method="post" class="form-inline">
				<p>
					...
					<select name="searchType" class="form-control">
						...
						<option ${param.searchType == '02' ? 'selected' : ''} value="02">제목</option>
					</select>
	...
	<table class="table table-bordered table-striped table-hover">
			<thead>
				<tr class="warning">
					...
					<th class="col-xs-1 text-center">좌석수</th>
			...
			<tbody>
				<c:if test="${not empty resrveList}" >
					<c:forEach var="resrve" items="${resrveList}" varStatus="i">
						<tr>
							...
							<td class="text-center">${resrve.mem_name}</td>
							<td class="text-center">${resrve.mem_id}</td>
							<td class="text-left">${resrve.movie_name}(${resrve.movie_ko_name})</td>
							<td class="text-center">${resrve.reservationcount}</td>
							<td class="text-right">
							<fmt:formatNumber var="pay_amount" value="${resrve.pay_amount}" pattern="#,###,###"/>
							${pay_amount}원
							</td>
							<td class="text-center">${resrve.pay_canel_yn == 'Y' ? '취소' : '완료'}</td>
							<td class="text-center">
								<fmt:parseDate value="${resrve.show_date}" pattern="yyyy-MM-dd" var="show" />
								<fmt:formatDate value="${now}" pattern="yyyyMMdd" var="nowDate" />
								<fmt:formatDate value="${show}" pattern="yyyyMMdd" var="showDate"/>
								<c:if test="${nowDate < showDate}">
									<input type="button" value="취소" class="btn btn-danger" onclick="fn_delete('${resrve.reservation_id}','${resrve.mem_point}','${resrve.mem_id}');">
								</c:if>
							</td>
						</tr>
					</c:forEach>			
				</c:if>
				<c:if test="${empty resrveList}" >
					<tr>
