...
<body>
	<div class="container main-container">
		...
		<div class="movie_story col-md-12">
				...
				<div class="movie_reservation_button col-md-12">
...
//						
		...
		$(function () {
			...
			$('[name=movieReviewInsertBtn]').click(function () { //댓글 등록 버튼 클릭시
				
					
				var rating  = $('#rating').html();
				console.log(rating);
				var insertData = $('#comment_content').val(); 
				console.log(insertData);
				movieReviewInsert(insertData,rating); //Insert 함수 호출
				
			
			})
			$(document).on('click','[name=movieReviewDeleteBtn]',function () { //댓글 삭제 버튼 클릭시
				
				 //var deleteData = $(this).parent().attr('id').substring(2); 
				
				 var deleteData = $(this).attr('data-id'); 
				console.log(deleteData);
				deleteMovieReview(deleteData); //Delete 함수 호출 
		 	...
		 	$(document).on('click','[name=movieReviewUpdateBtn]',function () { //댓글 수정 버튼 클릭시 
				...
				console.log(updateId,updateContent);
				/*
					$('<td>').remove("#td");
					$('<textarea>').attr('name','movie_review_content');*/
					//$('<input>').attr('type':'hidden','value:'+movie_id);
//					$('<input>').attr('type','hidden').attr('value',movie_id);
					...
					$('<textarea>').attr('name','movie_review_content').val(updateContent).css({ 
						...
						$('<span>').addClass('glyphicon glyphicon-share-alt').on('click',function(){
							 var updateForm = confirm("수정중인 내용은 저장되지 않습니다. 글 내용화면으로 돌아가시겠습니까?");
							 	...
							 	if(updateForm==true){
							 		 //location.reload();
							 		 //화면을 reload 하지 않고 하는 방법 
							 		 //reload->비효율적임 
							 		 $(this).closest('td').find('.viewDiv').show();
						...
						$editForm.appendTo($(this).closest('td'));
			...
			<table class="table table-bordered col-md-12" style="width:1300px;" id="review" >
				<%-- <c:forEach var="review" items="${movieReviewList}">
					<tr>
						<td><strong>${review.mem_id}</strong> 등록 : ${review.review_reg_date}</td>
					</tr>
						<tr>
							
									<td id="td${review.movie_review_id}" style="white-space: pre-wrap">${review.movie_review_content} <span id="commentUdt" onclick="fn_co_upt(,'<spring:escapeBody javaScriptEscape="true">${review.movie_review_content}</spring:escapeBody>');" class="glyphicon glyphicon-ok"> 
									 </span>  <span id="commentDel()" onclick="fn_co_del();" class="glyphicon glyphicon-remove"></span>
	</td>
						</tr>	 
	...
	// star rating
	  ...
	  <style>
				...
				<div class="movie_review col-md-2">
						
				<!-- 	<div id="movie_review_rating"> -->	
						...
						<div class="movie_review_content col-md-10">
							
							<%-- <c:if test="${empty }"> --%>
									<c:if test="${empty LOGIN_USER}">	
									<p>		
									<strong>로그인을 하시면 댓글을 등록할 수 있습니다</strong>
									</p>
									</c:if>	
									
								<%-- <h5>댓글 달기 ${ }</h5>			 --%>			
							<c:if test="${not empty LOGIN_USER}">	
							<input type="hidden" name="movie_review_id" value="${movie_review.movie_review_id}"/>
							<textarea class="form-control" rows="3" placeholder="내용을 입력해주세요" id="comment_content"></textarea>
							</c:if>
						</div>
						<c:if test="${not empty LOGIN_USER}">	
						<div class="movie_review_content_register col-md-4">
							
						  <button type="button" class="btn btn-primary" name="movieReviewInsertBtn">등록</button>
						 <!--  <button type="button" class="btn btn-success" name="movieReviewDeleteBtn">삭제</button> -->
						...
						<div id="templates" >
						<span class="star-input" id="starTemplate">
							<span class="input"> 
							    <input type="radio" name="star-input" id="p1" value="1"><label for="p1">0.5</label>
							    <input type="radio" name="star-input" id="p2" value="2"><label for="p2">1</label>
							    <input type="radio" name="star-input" id="p3" value="3"><label for="p3">1.5</label>
					  ...
					  <output  for="star-input"><b class="rating">0</b>점</output> 
							</span>
						</div>
