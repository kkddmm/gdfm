package kr.co.gdfm.common.util;
...
public class PagingUtil2 {
    ...
    public PagingUtil2(int currentPage, int totalCount, String mem_id) {
        this(currentPage, totalCount, 5,20, mem_id);
    }

    public PagingUtil2(int currentPage, int totalCount, int pageSize, int pageCount, String mem_id) {
        ...
        if (currentPage > pageCount) {
            pageHtml.append("<li><a href='#' onclick=\"fn_search(" + (startPage - 1) + ",'" + mem_id + "')\">이전</a></li>");
        }
        // 페이지 네비게이터 생성
        for (int i = startPage; i <= endPage; i++) {
            ...
            if (currentPage == i) {
                ...
                pageHtml.append(
                        "<li><a href='#' onclick=\"fn_search(" + i + ",'" + mem_id + "')\">" + i + "</a></li>");
            }
        }

        // 다음 생성
        if (totalPage - startPage >= pageCount) {
            pageHtml.append("<li><a href='#' onclick=\"fn_search(" + (endPage + 1) + ",'" + mem_id + "')\">다음</a></li>");
        }
